...
_Last reviewed: 2026-09-10._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`microsoft/playwright#42651`](https://github.com/microsoft/playwright/issues/42651) | cross-browser JSHandle property semantics | Firefox Juggler's `Runtime.getObjectProperties` explicitly walks `obj.proto` and collects each prototype's enumerable own properties, while the public API promises own property names and Chromium/WebKit request own properties only; existing cross-browser coverage checks instance fields but does not actually assert that inherited enumerable properties are excluded |
| [`actions/runner#4702`](https://github.com/actions/runner/issues/4702) | cross-runner shared-process ownership / destructive cleanup | orphan cleanup records a job-specific `RUNNER_TRACKING_ID`, snapshots pre-existing processes, then kills later processes carrying that job id; a per-user singleton such as `mspdbsrv.exe` can be created by one job and legitimately reused by a concurrent runner, so the origin tag is not sufficient proof of exclusive ownership at cleanup time |
| [`vitest-dev/vitest#11177`](https://github.com/vitest-dev/vitest/issues/11177) | dependency-optimizer resolution scope / mixed Vue compiler targets | the browser optimizer unconditionally registers a resolver for both `@vue/test-utils` and `@vue/compiler-core`, forcing `require.resolve()` for every matching compiler-core import; package-presence gating alone would still globally rewrite unrelated Vue compiler consumers whenever test-utils is installed, so the correction needs importer/dependency-edge scoping rather than only an installation check |
