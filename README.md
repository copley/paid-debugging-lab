...
_Last reviewed: 2026-09-20._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`microsoft/playwright#42804`](https://github.com/microsoft/playwright/issues/42804) | CLI boolean parsing / negated-option namespace collision | `install-browser` declares the literal boolean option `no-shell`, and the generated help metadata therefore registers `no-shell`. The wrapper's custom minimist parser rewrites every `--no-*` token to the base key before validation, so `--no-shell` becomes unknown `shell=false` even though the downstream Playwright install CLI already supports the negated option correctly |
| [`vitest-dev/vitest#11315`](https://github.com/vitest-dev/vitest/issues/11315) | Concurrent initialization / shared credential publication race | `resolveTokenFromPath()` uses an `existsSync()` check followed by a direct `writeFileSync()` to the shared final token path and trusts any existing file after `.trim()`. Concurrent first-start processes can therefore observe the destination during publication and accept an empty token; browser mode then falls back to a different token value and times out during WebSocket authentication |
| [`cloudflare/workers-sdk#15719`](https://github.com/cloudflare/workers-sdk/issues/15719) | Cross-context RPC lifecycle / teardown deadlock | The Vitest worker buffers `onUserConsoleLog` RPC messages when a cross-Durable-Object I/O error prevents immediate send, but it keeps the original call correlation state. If the log is parked during the final task round trip, `rpcDone()` can await that call while the next message that would flush the buffer is itself blocked behind teardown; Cloudflare's automated triage independently reproduced the hang |
