...
_Last reviewed: 2026-09-11._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`microsoft/playwright#42669`](https://github.com/microsoft/playwright/issues/42669) | BiDi console-value deserialization / driver crash | BiDi `createHandle()` already computes a safe protocol-level preview, but by-value console arguments reach `JSHandle` without an object id; the constructor then ignores that preview and calls `String(value)`. The BiDi deserializer maps function-valued properties such as `toString` and `valueOf` to `undefined`, so a logged object can become non-coercible and throw inside the driver before `_setPreview(preview)` runs |
| [`actions/runner#4692`](https://github.com/actions/runner/issues/4692) | workflow-command parsing / untrusted log-text interpretation | the modern `::command::` parser trims leading whitespace and requires the command marker at the start, while legacy `##[command]` uses `IndexOf(Prefix)` and therefore accepts the marker anywhere in a line; printed third-party text can consequently be interpreted as an annotation or stateful runner command rather than ordinary log text |
| [`actions/runner#4703`](https://github.com/actions/runner/issues/4703) | broker job acquisition / unbounded lifecycle wait | the message-loop token is linked to runner shutdown rather than a per-acquisition deadline, and `RunServer.GetJobMessageAsync()` forwards that token unchanged into the HTTP call inside `RetryRequest`; if the underlying request neither completes nor faults, the retry layer never receives an exception and the listener has no independent deadline to regain progress |
