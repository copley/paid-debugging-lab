...
_Last reviewed: 2026-09-23._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`microsoft/playwright#42857`](https://github.com/microsoft/playwright/issues/42857) | Python 3.14 deferred-annotation evaluation / callback introspection | `playwright-python` calls `inspect.signature()` in two runtime arity-inspection paths and then uses only parameter count/kinds. On Python 3.14, signature construction can evaluate deferred annotations even though Playwright does not need them; `annotation_format=annotationlib.Format.STRING` avoids that side effect while preserving the parameter metadata Playwright actually consumes |
| [`vitest-dev/vitest#11329`](https://github.com/vitest-dev/vitest/issues/11329) | VM-pool dependency optimizer / unrelated app entrypoint contamination | Vitest creates `__vitest_vm__` as a Vite `client` consumer, then its `configEnvironment` hook returns before applying the optimizer normalization used by other test environments. Vite therefore retains client-side dependency discovery and can crawl `index.html`, so an application-only unbundlable dependency can fail a VM test run even when no test imports it |
| [`cloudflare/workers-sdk#15788`](https://github.com/cloudflare/workers-sdk/issues/15788) | Workflow timeout lifecycle / active native timer leak | Current `workflows-shared` wraps `scheduler.wait()` so an abort resolves the wrapper promise, but the underlying scheduled wait is still created without the abort signal. Completed steps can therefore release JavaScript control while leaving the native timer alive, matching the reported 10,000-active-timeout exhaustion. The repair needs cancellation of the native wait, not only early resolution of its wrapper |
