...
_Last reviewed: 2026-09-22._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`actions/runner#3415`](https://github.com/actions/runner/issues/3415) | Built-in shell fallback / runner-doc contract drift | The current Windows default-shell branch probes `pwsh` and falls back to PowerShell Desktop, but the explicit-shell branch calls `Which(..., require: true)` immediately. GitHub's current workflow syntax still documents `pwsh → powershell` on self-hosted Windows and `bash → sh` on non-Windows, so exact built-in shell keywords bypass a documented fallback that custom shell templates should not inherit |
| [`microsoft/playwright#42849`](https://github.com/microsoft/playwright/issues/42849) | Selector execution-world loss / custom-engine regression | Selector parsing records `world: main` for custom engines registered without `contentScript: true`, but `HighlightController` keeps only the resolved frame and always evaluates highlights in the utility world. Page-global-dependent custom engines therefore work for ordinary locator actions but fail for `locator.highlight()` |
| [`docker/cli#7264`](https://github.com/docker/cli/issues/7264) | Multi-file stack merge / field-presence loss | Stack files are normalized into typed secret configs before top-level merge. An override containing only `name` reaches `loadFileObjectConfig()` with an empty `file`, which becomes the working directory through `absPath`, while the later typed merge no longer knows which fields were omitted. This explains both the synthetic `file: /cwd` and loss of the base `external: true` state |
