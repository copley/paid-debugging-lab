...
_Last reviewed: 2026-09-15._ These are source-inspected candidates in the engineering queue, not claims of contribution.

| Upstream issue | Failure class | Current evidence |
| --- | --- | --- |
| [`apollographql/apollo-client#13463`](https://github.com/apollographql/apollo-client/issues/13463) | API/cache lifecycle race / lost notification | `ObservableQuery` snapshots `dirty` and then calls `resetNotifications()` before applying the `activeOperations` gate. If a cache update arrives after a network result is delivered but before that operation finalizes, the gate returns with the dirty state already cleared. Operation finalization only removes the operation, so no deferred cache notification is guaranteed; the normalized cache can be correct while `useQuery` remains stale |
| [`vitest-dev/vitest#11265`](https://github.com/vitest-dev/vitest/issues/11265) | Browser-mode environment/define split / JSX runtime mismatch | In browser mode `TestConfigPlugin` deliberately leaves Vite `define` entries in place and skips `deleteDefineConfig()`, while non-browser mode hoists `process.env.*` define values into the real process environment. Vitest 4 ran that hoist while separately preserving the original browser define map. A production `process.env.NODE_ENV` define can therefore make application transforms and pre-bundled React JSX runtime choose different branches |
| [`cloudflare/workers-sdk#15644`](https://github.com/cloudflare/workers-sdk/issues/15644) | Remote RPC capability proxy / nested API loss | AI Search remote bindings use the generic Cap'n Web remote proxy. Existing AI Search remote-binding E2E coverage exercises namespace create/delete and instance `info()` but not the nested `instance.items` capability. The report shows top-level RPC working while `items` arrives as a callable stub without `list/upload/delete`, pointing at nested capability forwarding/serialization rather than Wrangler binding configuration |
